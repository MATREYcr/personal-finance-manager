...
4. **Categories** (`Categories.jsx`) — table of Name/Type(badge)/Actions (same solid edit/delete icon buttons). Clicking Delete on a category with transactions shows an inline red-tinted alert row: *"Cannot delete a category that still has transactions. Reassign or delete those transactions first."* "New category" Dialog: Name, Type.
