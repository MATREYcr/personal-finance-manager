    ...
    <div className="space-y-4">
      ...
      <div className="w-full overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {(rules as RecurringTransactionWithCategory[] | undefined)?.map(
              (rule) => (
                <TableRow key={rule.id}>
                  ...
                  <TableCell className="text-right">
                    {Number(rule.amount).toFixed(2)} {rule.currency}
                  </TableCell>
                  <TableCell>{t(frequencyKey[rule.frequency])}</TableCell>
                  {/* nextRunDate is stored as UTC midnight; format in UTC or a negative offset rolls the day back. */}
                  <TableCell>
                    {new Date(rule.nextRunDate).toLocaleDateString(undefined, {
                      timeZone: 'UTC',
                    })}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="space-x-2 text-right">
                    ...
                    {rule.active ? (
                      <Button
                        ...
                        onClick={() =>
                          toggleActive.mutate({ id: rule.id, active: false })
                        }
                      >
                      ...
                      <Button
                        ...
                        disabled={toggleActive.isPending}
                        onClick={() =>
                          toggleActive.mutate({ id: rule.id, active: true })
                        }
                      >
